...
class rli_parse {
	...
	private function load_members() {
		global $rli;
		$members = array();
		$member_data = $rli->get_cache_data('member');
			...
			foreach($member_data as $key => $member) {
				if($k == $key AND !$mem['delete']) {
					...
					if($rli->config('member_display') == 2) {
						$members[$key]['raid_list'] = $rli->raid->get_memberraids($mem['times']);
						$a = $rli->raid->calc_att($mem['times']);
						$members[$key]['att_dkp_begin'] = $a['att_dkp_begin'];
						$members[$key]['att_dkp_end'] = $a['att_dkp_end'];
						unset($a);
					} else {
					...
					if($members[$key]['raid_list']) {
						$raids = $members[$key]['raid_list'];
						...
						foreach($raids as $raid_id) {
							if(($raid_id != $rli->add_data['att_begin_raid'] AND $raid_id != $rli->add_data['att_end_raid']) OR !$rli->config('attendence_raid')) {
								...
								$dkp = round($dkp, 2);
								if($dkp <  $raid['value']) { //add an adjustment
									$dkp -= $raid['value'];

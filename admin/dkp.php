...
class raidlogimport extends EQdkp_Admin
{
	...
	function process_members()
	{
		...
		foreach($data['members'] as $key => $member)
		{
	        ...
	        {
	        	$raidlist = '<td>'.$jquery->MultiSelect('members['.$key.'][raid_list]', $rli->raidlist($data['raids']), $member['raid_list'], '200', '200', false, 'members_'.$key.'_raidlist').'</td>';
	        }
	        else
	        {
	        	$raidlist = ($rli->config['member_display']) ? $rli->get_checkraidlist($data['raids'], $member['raid_list'], $key) : '';
	        }
